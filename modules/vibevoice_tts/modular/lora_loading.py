...
def _resolve_adapter_root(checkpoint_path):
...
def _load_connector(module, path, device):
...
def _load_diffusion_head(model, adapter_root, device, report):
...
def _load_language_model(model, adapter_root, device, report):
    ...
    # VibeVoice's language_model is a Qwen2Model (loaded via AutoModel),
    # not a CausalLM.  Externally-trained LoRAs often set task_type to
    # CAUSAL_LM, which makes PEFT create a PeftModelForCausalLM that
    # requires prepare_inputs_for_generation — a method Qwen2Model lacks.
    # Detect this mismatch and override to FEATURE_EXTRACTION.
    peft_config = PeftConfig.from_pretrained(str(adapter_root))
    if str(peft_config.task_type) in ("CAUSAL_LM", "TaskType.CAUSAL_LM"):
        if not hasattr(model.model.language_model, "prepare_inputs_for_generation"):
            logger.info(
                "Overriding adapter task_type CAUSAL_LM -> FEATURE_EXTRACTION "
                "(language_model is not a CausalLM)"
            )
            peft_config.task_type = "FEATURE_EXTRACTION"

    peft_lm = PeftModel.from_pretrained(
        model.model.language_model, adapter_root, config=peft_config
    )
    peft_lm.to(device)
...
def load_lora_assets(model, checkpoint_dir, device=None):
    ...
    if not any([report.language_model, report.diffusion_head_lora,
                report.diffusion_head_full, report.acoustic_connector,
                report.semantic_connector]):
        logger.warning(
            "No adapter assets were loaded. Ensure the checkpoint directory is "
            "correct and contains LoRA weights."
        )
