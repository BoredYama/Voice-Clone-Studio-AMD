...
from transformers.generation import (
    GenerationMixin, GenerationConfig, LogitsProcessor,
...
def _update_model_kwargs_for_generation(outputs, model_kwargs, num_new_tokens=1):
    """Update model_kwargs after adding new tokens (for multi-token prefill)."""
...
class VibeVoiceStreamingForConditionalGenerationInference(VibeVoiceStreamingPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None,
                                       cache_position=None, inputs_embeds=None, use_cache=True, **kwargs):
        """Simplified version that works with any cache type (old/new DynamicCache, tuples)."""
        model_inputs = {}
    ...
    def forward_tts_lm(
        ...
        return_dict=None,
        cache_position=None,
        lm_last_hidden_state=None,
        tts_text_masks=None,
        **kwargs,
    ):
        """Single pass of the TTS LM with hidden state splicing and type embedding."""
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
    ...
    def _build_generate_config_model_kwargs(self, generation_config, inputs, tokenizer,
                                             return_processors=False, **kwargs):
        # Extract max_new_tokens from kwargs so it goes into GenerationConfig
        ...
        self._prepare_cache_for_generation(
            generation_config, model_kwargs, None, batch_size, max_cache_length
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        Returns:
            VibeVoiceGenerationOutput with sequences, speech_outputs, reach_max_step_sample
        """
        ...
        negative_kwargs = {
            'input_ids': torch.full(
                (kwargs['input_ids'].shape[0], 1), neg_text_input_id,
        ...
        # Convert legacy caches from .pt file to proper DynamicCache objects.
        # The .pt files may contain old-version DynamicCache (with key_cache/value_cache
        # instead of layers) or raw tuples -- both need conversion.
        from transformers import DynamicCache
        for out in (outputs, tts_lm_outputs, negative_outputs, tts_lm_negative_outputs):
            if not hasattr(out, "past_key_values") or out.past_key_values is None:
                continue
            pkv = out.past_key_values
            if isinstance(pkv, tuple):
                out.past_key_values = DynamicCache.from_legacy_cache(pkv)
            elif isinstance(pkv, DynamicCache) and not hasattr(pkv, 'layers'):
                # Old transformers DynamicCache loaded from pickle -- has key_cache/value_cache
                legacy = tuple(
                    (pkv.key_cache[i], pkv.value_cache[i])
                    for i in range(len(pkv.key_cache))
                )
                out.past_key_values = DynamicCache.from_legacy_cache(legacy)
        ...
        while True:
            ...
            cur_input_tts_text_ids = tts_text_ids[
                :, tts_text_window_index * TTS_TEXT_WINDOW_SIZE:(tts_text_window_index + 1) * TTS_TEXT_WINDOW_SIZE
            ]
            next_text_window_size = tts_text_ids[
                :, (tts_text_window_index + 1) * TTS_TEXT_WINDOW_SIZE:(tts_text_window_index + 2) * TTS_TEXT_WINDOW_SIZE
            ].shape[1]
            ...
            for cur_speech_index in range(TTS_SPEECH_WINDOW_SIZE):
                ...
                tts_eos_logits = torch.sigmoid(
                    self.tts_eos_classifier(tts_lm_outputs.last_hidden_state[diffusion_indices, -1, :])
                )
